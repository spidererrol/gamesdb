...
import { array2map, cloneMap, EditMapItem, formap, haveBlank, isKnown, makeCloudItemsSettable, map2object, mapmap, newNSMap, NSMap, OwnershipInfo } from "../../libs/utils"
...
function AddGame(props: AGProps): JSX.Element {
    ...
    // let nexti = useRef(0)
    const [navElement, setNavElement] = useState<anyElement>()
    ...
    if (isKnown(navElement))
        return navElement as JSX.Element
    ...
    return <fieldset className="game">
        ...
        <hr />
        <input type="submit" value="create" onClick={save} /><button hidden={!props.myuser.get.isAdmin} onClick={dumpcurrent}>debug</button>
    </fieldset>
}
